...
| `aura run --trigger on-merge` | Override trigger mode |
...
| 1 | Config error (missing file, bad YAML, missing required env var) |
| 2 | Every connector failed — nothing to summarize |
| 3 | Agent error (LLM call failed after retries) or write failed |

## Trigger modes

| Mode | When it fires | Lookback window |
|---|---|---|
| `weekly` | CI cron (default: Fri 17:00 UTC) | Last 7 days |
| `on-merge` | Every merged PR | Since last brief, or 24h fallback |
| `both` | Both triggers in one workflow | Branched by `AURA_TRIGGER` env var |

## Example positioning

You can use any of these for internal sharing:

> AURA turns project signals into a weekly brief.
>
> A simple agentic method for weekly Scrum visibility.
>
> Weekly project reviews, generated from real work.
>
> From code, issues, and KPIs to a readable sprint brief.
>
> A living weekly project brief powered by agents.

## Technical model

AURA is implemented as a reusable Pydantic AI agent pipeline:

- A **Pydantic AI orchestrator** coordinates three subagents
- **Provider-neutral model config** — use Anthropic (`anthropic:...`) or OpenAI
  (`openai:...`) model strings without changing the pipeline
- **Source connectors** for GitHub, Azure DevOps (pluggable; add your own)
- **Typed outputs** end-to-end — every connector emits a validated Pydantic model
  and every agent response is parsed into shared Pydantic schemas
- **Scheduled triggers** via your CI's cron (GitHub Actions or Azure Pipelines)
- **Graceful degradation** — a failing connector becomes a warning in the brief, not
  an aborted run
- **Locked core rules** in [`aura.core.md`](openaura/instructions/aura.core.md) — a
  maintainer-owned system prompt enforcing evidence-first, no fabrication, no hidden
  blockers

## Open method

AURA is intended to be simple, adaptable, and portable. Teams should be able to:

- adopt the method without changing their stack
- define their own KPI profile
- customize the weekly brief format
- run it with their own agent infrastructure

The core method is the method. The Python package is one implementation.

## Repository CI/CD

OpenAURA itself is intended to live on GitHub, so the project has first-party GitHub
automation under `.github/`:

- `.github/workflows/ci.yml` — PR and merge-queue checks for `main`: lint, format check,
  mypy, Bandit, tests, coverage, dependency audit, package build, `twine check`, and
  wheel-install smoke testing.
- `.github/workflows/codeql.yml` — CodeQL security analysis.
- `.github/workflows/scorecard.yml` — OpenSSF Scorecard analysis.
- `.github/workflows/release.yml` — tagged releases to PyPI via Trusted Publishing,
  provenance attestations, Sigstore signing, and GitHub Releases.
- `.github/release.yml` — categories and exclusions for GitHub's automatically generated
  release notes.
- `.github/dependabot.yml` — dependency and GitHub Actions update PRs.

The files in `openaura/templates/` are not this repo's own CI/CD. They are reusable
workflow templates that OpenAURA installs into downstream projects that want generated
AURA briefs.

## Security posture

OpenAURA is a public, open-source project and is hardened accordingly:
...
- Releases to PyPI via **Trusted Publishing** (OIDC, no long-lived tokens) with
  SLSA build provenance attestations
- Release artifacts are Sigstore-signed
- Tokens redacted in all log output
- Connector base URLs must be `https://`

See [`SECURITY.md`](SECURITY.md) for the full vulnerability disclosure policy.

## Closing

AURA helps teams create a weekly source of truth from the work they are already doing.

> Not another dashboard. Not another reporting ritual.
> A simple agentic brief for project and product teams.
>
